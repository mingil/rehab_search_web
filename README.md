...
```bash
git clone https://github.com/YOUR_GITHUB_USERNAME/rehab_search_web.git
cd rehab_search_web
```
...
```
*(If deployed on a NAS, replace `localhost` with your NAS IP address)*
...
- **Memory Optimized:** Features robust retry-logic for API fetching and prevents memory leaks during PDF parsing. Docker memory is securely limited to 8GB to prevent host system crashes (OOM) on Synology NAS.
